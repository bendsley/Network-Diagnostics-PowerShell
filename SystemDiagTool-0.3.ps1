...
function Invoke-Users {
    ...
    # ── C. Entra ID — Device Join Status (dsregcmd, no modules) ──
    Write-LogSection "Entra ID (Azure AD) — Device Join Status" -NoScreen:$NoScreen
    try {
        $dsreg = dsregcmd /status 2>&1

        # Parse key fields
        $parseField = { param($pattern)
            $line = $dsreg | Where-Object { $_ -match $pattern } | Select-Object -First 1
            if ($line -match ':\s*(.+)$') { return $Matches[1].Trim() } else { return 'N/A' }
        }

        $azureAdJoined     = & $parseField 'AzureAdJoined'
        $enterpriseJoined  = & $parseField 'EnterpriseJoined'
        $domainJoined      = & $parseField 'DomainJoined'
        $workplaceJoined   = & $parseField 'WorkplaceJoined'
        $tenantName        = & $parseField 'TenantName'
        $tenantId          = & $parseField 'TenantId'
        $deviceId          = & $parseField 'DeviceId'
        $mdmEnrolled       = & $parseField 'MdmUrl'

        # Determine join type label
        $joinType = switch ($true) {
            ($azureAdJoined -eq 'YES' -and $domainJoined -eq 'YES') { "Hybrid Entra Joined (AD + Entra)"  }
            ($azureAdJoined -eq 'YES')                               { "Entra Joined (cloud-only)"         }
            ($workplaceJoined -eq 'YES')                             { "Workplace Joined (Entra Registered)" }
            ($domainJoined -eq 'YES')                                { "Domain Joined only (no Entra)"     }
            default                                                  { "Not joined to Entra or domain"     }
        }

        Write-Log "  Join Type         : $joinType"                                                          -NoScreen:$NoScreen
        Write-Log "  Entra Joined      : $azureAdJoined"                                                     -NoScreen:$NoScreen
        Write-Log "  Domain Joined     : $domainJoined"                                                      -NoScreen:$NoScreen
        Write-Log "  Workplace Joined  : $workplaceJoined"                                                   -NoScreen:$NoScreen
        Write-Log "  Tenant Name       : $tenantName"                                                        -NoScreen:$NoScreen
        Write-Log "  Tenant ID         : $tenantId"                                                          -NoScreen:$NoScreen
        Write-Log "  Device ID         : $deviceId"                                                          -NoScreen:$NoScreen
        Write-Log "  MDM Enrolled      : $(if ($mdmEnrolled -ne 'N/A' -and $mdmEnrolled -ne '') { 'Yes — ' + $mdmEnrolled } else { 'No' })" -NoScreen:$NoScreen

    } catch {
        Write-Log "  [!] dsregcmd failed: $_" -NoScreen:$NoScreen
    }

    # ── D. Entra — Cached local profiles from Entra sign-ins ─────
    Write-LogSection "Entra ID — Locally Cached Sign-in Profiles" -NoScreen:$NoScreen
    Write-Log "  (Users who have signed in with an Entra/Microsoft account on this machine)" -NoScreen:$NoScreen
    Write-Log "" -NoScreen:$NoScreen
    try {
        # Registry path where Windows caches Entra/MSA identity tokens
        $idCacheBase = "HKLM:\SOFTWARE\Microsoft\IdentityStore\Cache"
        if (Test-Path $idCacheBase) {
            $cacheKeys = Get-ChildItem -Path $idCacheBase -ErrorAction SilentlyContinue
            $foundProfiles = 0
            foreach ($key in $cacheKeys) {
                ...
                foreach ($sub in $subKeys) {
                    ...
                    $upn   = $props.UserName ?? $props.ProviderID ?? $null
                    if ($upn -and $upn -match '@') {
                        $marker = if ($upn -eq "$($env:USERNAME)@*" -or
                                      $upn -match [regex]::Escape($env:USERNAME)) {
                                      "  ◄ CURRENT USER" } else { "" }
                        Write-Log "  Entra Account     : $upn$marker" -NoScreen:$NoScreen
                        $foundProfiles++
                    }
                }
            }
            if ($foundProfiles -eq 0) {
                Write-Log "  No cached Entra sign-in profiles found in identity store." -NoScreen:$NoScreen
            } else {
                Write-Log "" -NoScreen:$NoScreen
                Write-Log "  Total cached Entra profiles: $foundProfiles" -NoScreen:$NoScreen
            }
        } else {
            Write-Log "  Identity store cache not present on this machine." -NoScreen:$NoScreen
        }

        # Also check user profile folders for Entra-style names (email@domain format)
        $profileFolders = Get-ChildItem -Path "$env:SystemDrive\Users" -Directory -ErrorAction SilentlyContinue |
                          Where-Object { $_.Name -match '.+\..+\..+' -or $_.Name -match '@' }
        if ($profileFolders) {
            Write-Log "" -NoScreen:$NoScreen
            Write-Log "  Profile folders with Entra-style names:" -NoScreen:$NoScreen
            foreach ($pf in $profileFolders) {
                Write-Log "    $($pf.FullName)" -NoScreen:$NoScreen
            }
        }
    } catch {
        Write-Log "  [!] Could not enumerate cached Entra profiles: $_" -NoScreen:$NoScreen
    }

    # ── E. Entra — Currently logged-on Entra users on this machine ─
    Write-LogSection "Entra ID — Currently Logged-On Users (This Machine)" -NoScreen:$NoScreen
    try {
        # query user / quser shows all active and disconnected sessions
        $quserOutput = query user 2>&1
        $activeSessions = [System.Collections.Generic.List[PSCustomObject]]::new()

        if ($quserOutput -and $quserOutput -notmatch 'No User exists') {
            # Skip the header line and parse each session
            $quserOutput | Select-Object -Skip 1 | ForEach-Object {
                $line = $_ -replace '\s+', ' ' | ForEach-Object { $_.Trim() }
                if ($line) {
                    # quser columns: USERNAME, SESSIONNAME, ID, STATE, IDLE TIME, LOGON TIME
                    $parts = $line -split '\s+'
                    if ($parts.Count -ge 4) {
                        $activeSessions.Add([PSCustomObject]@{
                            Username    = $parts[0] -replace '^>', ''   # '>' marks current session
                            Session     = $parts[1]
                            ID          = $parts[2]
                            State       = $parts[3]
                            LogonTime   = if ($parts.Count -ge 7) { "$($parts[5]) $($parts[6])" } else { "N/A" }
                        })
                    }
                }
            }
        ...
        if ($activeSessions.Count -eq 0) {
            Write-Log "  No active user sessions detected via query user." -NoScreen:$NoScreen
        } else {
            Write-Log "  Active / disconnected sessions on this machine:" -NoScreen:$NoScreen
            Write-Log "" -NoScreen:$NoScreen
            foreach ($s in $activeSessions) {
                $marker = if ($s.Username -eq $env:USERNAME) { "  ◄ CURRENT USER" } else { "" }
                Write-Log "  User         : $($s.Username)$marker" -NoScreen:$NoScreen
                Write-Log "    Session    : $($s.Session)" -NoScreen:$NoScreen
                Write-Log "    Session ID : $($s.ID)"      -NoScreen:$NoScreen
                Write-Log "    State      : $($s.State)"   -NoScreen:$NoScreen
                Write-Log "    Logon Time : $($s.LogonTime)" -NoScreen:$NoScreen
                Write-Log "" -NoScreen:$NoScreen
            }
        }
    } catch {
        Write-Log "  [!] query user failed: $_" -NoScreen:$NoScreen
    }

    # Cross-reference sessions against Entra identity store to flag Entra-backed accounts
    Write-Log "" -NoScreen:$NoScreen
    Write-Log "  Checking which logged-on users are Entra-backed..." -NoScreen:$NoScreen
    try {
        $idCacheBase = "HKLM:\SOFTWARE\Microsoft\IdentityStore\Cache"
        $entraUpns   = [System.Collections.Generic.List[string]]::new()

        if (Test-Path $idCacheBase) {
            foreach ($key in Get-ChildItem -Path $idCacheBase -ErrorAction SilentlyContinue) {
                foreach ($sub in Get-ChildItem -Path $key.PSPath -ErrorAction SilentlyContinue) {
                    $props = Get-ItemProperty -Path $sub.PSPath -ErrorAction SilentlyContinue
                    $upn   = $props.UserName ?? $props.ProviderID ?? $null
                    if ($upn -and $upn -match '@') { $entraUpns.Add($upn) }
                }
            }
        }

        # Also pull Entra UPNs from the AAD Broker plugin registry (more reliable on Entra-joined machines)
        $brokerPath = "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList"
        $profileKeys = Get-ChildItem -Path $brokerPath -ErrorAction SilentlyContinue
        foreach ($pk in $profileKeys) {
            $ppProps = Get-ItemProperty -Path $pk.PSPath -ErrorAction SilentlyContinue
            if ($ppProps.FullProfile -eq 1) {
                $sid = $pk.PSChildName
                $upnPath = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Authentication\LogonUI\SessionData"
                # Check HKCU hive for the SID to find Entra UPN
                try {
                    $hivePath = "Registry::HKEY_USERS\$sid\SOFTWARE\Microsoft\Windows\CurrentVersion\AAD\Package"
                    if (Test-Path $hivePath) {
                        $aadProps = Get-ItemProperty -Path $hivePath -ErrorAction SilentlyContinue
                        if ($aadProps.Username -and $aadProps.Username -match '@') {
                            if ($aadProps.Username -notin $entraUpns) {
                                $entraUpns.Add($aadProps.Username)
                            }
                        }
                    }
                } catch { }
        ...
        if ($entraUpns.Count -gt 0) {
            Write-Log "" -NoScreen:$NoScreen
            Write-Log "  Entra-backed accounts with profiles on this machine:" -NoScreen:$NoScreen
            foreach ($upn in ($entraUpns | Sort-Object -Unique)) {
                $marker = if ($upn -match [regex]::Escape($env:USERNAME)) { "  ◄ CURRENT USER" } else { "" }
                Write-Log "    $upn$marker" -NoScreen:$NoScreen
            }
            Write-Log "" -NoScreen:$NoScreen
            Write-Log "  Total Entra-backed accounts: $($($entraUpns | Sort-Object -Unique).Count)" -NoScreen:$NoScreen
        } else {
            Write-Log "  No Entra-backed accounts found in identity store." -NoScreen:$NoScreen
        }
    } catch {
        Write-Log "  [!] Could not check Entra identity store: $_" -NoScreen:$NoScreen
    }
}
