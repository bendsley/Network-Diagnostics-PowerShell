...
function Invoke-Users {
    ...
    # ── E. Entra — Full directory via Microsoft.Graph (optional) ─
    Write-LogSection "Entra ID — Full Directory (Microsoft.Graph)" -NoScreen:$NoScreen
    ...
    if (-not $graphAvailable) {
        Write-Log "  Microsoft.Graph module not installed — skipping full directory query." -NoScreen:$NoScreen
        Write-Log "  To install: Install-Module Microsoft.Graph -Scope CurrentUser"        -NoScreen:$NoScreen
    } else {
        Write-Log "  Microsoft.Graph module found." -NoScreen:$NoScreen

        # Only prompt for auth when running interactively (not in NoScreen/silent mode)
        if (-not $NoScreen) {
            Write-Host ""
            Write-Host "  Microsoft.Graph is available. Query full Entra user directory?" -ForegroundColor Yellow
            Write-Host "  This will open a browser authentication prompt." -ForegroundColor DarkGray
            $graphChoice = Read-Host "  Proceed with Entra directory query? (Y/N)"
        } else {
            $graphChoice = 'N'   # Never prompt in silent/log-only mode
        }

        if ($graphChoice -match '^[Yy]$') {
            try {
                # Connect with minimal required scope — read-only user list
                Connect-MgGraph -Scopes "User.Read.All" -ErrorAction Stop | Out-Null
                Write-Log "  Connected to Microsoft Graph." -NoScreen:$NoScreen
                Write-Log "" -NoScreen:$NoScreen

                $mgUsers = Get-MgUser -All -Property Id, DisplayName, UserPrincipalName,
                                            AccountEnabled, CreatedDateTime, LastPasswordChangeDateTime,
                                            JobTitle, Department, Mail, UserType,
                ...
                foreach ($u in $mgUsers) {
                    $marker     = if ($u.UserPrincipalName -eq $currentUpn) { "  ◄ CURRENT USER" } else { "" }
                    $status     = if ($u.AccountEnabled) { "Enabled" } else { "Disabled" }
                    $userType   = $u.UserType ?? "Member"
                    $synced     = if ($u.OnPremisesSyncEnabled) { "Hybrid (synced from AD)" } else { "Cloud-only" }
                    $licensed   = if ($u.AssignedLicenses.Count -gt 0) { "Yes ($($u.AssignedLicenses.Count) license(s))" } else { "No" }
                    $created    = if ($u.CreatedDateTime) { ([datetime]$u.CreatedDateTime).ToString('yyyy-MM-dd') } else { "N/A" }
                    $pwChanged  = if ($u.LastPasswordChangeDateTime) { ([datetime]$u.LastPasswordChangeDateTime).ToString('yyyy-MM-dd') } else { "N/A" }

                    Write-Log "  [$status] $($u.UserPrincipalName)$marker"     -NoScreen:$NoScreen
                    if ($u.DisplayName)  { Write-Log "    Display Name  : $($u.DisplayName)"  -NoScreen:$NoScreen }
                    if ($u.JobTitle)     { Write-Log "    Title         : $($u.JobTitle)"      -NoScreen:$NoScreen }
                    if ($u.Department)   { Write-Log "    Department    : $($u.Department)"    -NoScreen:$NoScreen }
                    if ($u.Mail)         { Write-Log "    Mail          : $($u.Mail)"          -NoScreen:$NoScreen }
                    Write-Log "    User Type     : $userType"   -NoScreen:$NoScreen
                    Write-Log "    Sync Status   : $synced"     -NoScreen:$NoScreen
                    Write-Log "    Licensed      : $licensed"   -NoScreen:$NoScreen
                    Write-Log "    Created       : $created"    -NoScreen:$NoScreen
                    Write-Log "    PW Changed    : $pwChanged"  -NoScreen:$NoScreen
                    Write-Log "" -NoScreen:$NoScreen
                }

                Write-Log "  Total Entra users: $($mgUsers.Count)" -NoScreen:$NoScreen

                # Disconnect cleanly
                Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null
                Write-Log "  Disconnected from Microsoft Graph." -NoScreen:$NoScreen

            } catch {
                Write-Log "  [!] Microsoft.Graph query failed: $_" -NoScreen:$NoScreen
                try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch { }
            }
        } else {
            Write-Log "  Entra directory query skipped." -NoScreen:$NoScreen
        }
    }
}
